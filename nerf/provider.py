...
class NeRFDataset:
    def __init__(self, opt, device, type='train', n_test=10):
        ...
        if self.mode == 'colmap' and type == 'test':
            ...
            for f in tqdm.tqdm(frames, desc=f'Loading {type} data'):
                ...
                if use_PIL:
                    ...
                    image_shape1, image_shape0 = image.size
                else:
                    image = cv2.imread(f_path, cv2.IMREAD_UNCHANGED) # [H, W, 3] o [H, W, 4]
                    image_shape1, image_shape0 = image.shape[1], image.shape[0]


                if self.H is None or self.W is None:
                    self.H = image_shape0 // self.downscale
                    self.W = image_shape1 // self.downscale
                ...
                if image_shape0 != self.H or image_shape1 != self.W:
                    if not use_PIL:
                ...
                #pyexr.write(f'./test_image/test.exr', image/65535)
                self.poses.append(pose)
        ...
        #pyexr.write(f'./test_image/test.exr', (self.images[0,:,:,:]/255).cpu().numpy())
